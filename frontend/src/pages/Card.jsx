...
const Card = () => {
    ...
    return (
        <div className='font-thai'>
            <title>ตะกร้าสินค้า - Micro Agrotec</title>
           <Header/>
           <section className='bg-[url("http://localhost:3000/images/banner/shop.png")] h-[220px] mt-6 bg-cover bg-no-repeat relative bg-left'>
            <div className='absolute left-0 top-0 w-full h-full bg-[#2422228a]'>
                <div className='w-[85%] md:w-[80%] sm:w-[90%] lg:w-[90%] h-full mx-auto'>
                    <div className='flex flex-col justify-center gap-1 items-center h-full w-full text-white'>
                <h2 className='text-3xl font-bold'>หน้าตะกร้าสินค้า</h2>
    ...
    <div className='w-[85%] lg:w-[90%] md:w-[90%] sm:w-[90%] mx-auto py-16'>
        {
            card_products.length > 0 || outofstock_products > 0 ? <div className='flex flex-wrap'>
                <div className='w-[67%] md-lg:w-full'>
                    ...
                    <h2 className='text-md text-blue-800 font-semibold'>สินค้าสต๊อก {card_products.length}</h2>
                            </div>
   ...
   <div className='flex justify-between w-5/12 sm:w-full sm:mt-3'>
       <div className='pl-4 sm:pl-0'>
           <h2 className='text-lg text-orange-500'>{pt.productInfo.price - Math.floor((pt.productInfo.price * pt.productInfo.discount) / 100)} บาท</h2>
       ...
       <div className='flex gap-2 flex-col'>
           ...
           <button onClick={() => dispatch(delete_card_product(pt._id)) } className='px-5 py-[3px] bg-red-500 text-white'>ลบ</button>
       </div>
   </div>
...

                {
                    outofstock_products.length > 0 && <div className='flex flex-col gap-3'>
                         <div className='bg-white p-4'>
                    <h2 className='text-md text-red-500 font-semibold'>หมดไปแล้ว {outofstock_products.length}</h2>
                            </div>
                  ...
                  <div className='bg-white p-4'>
                  {
                       ...
                       <span className='text-sm'>แบรนด์: {p.products[0].brand}</span>
                       </div>
                           </div>
                       </div>
   ...
   <div className='flex justify-between w-5/12 sm:w-full sm:mt-3'>
       <div className='pl-4 sm:pl-0'>
           <h2 className='text-lg text-orange-500'>{p.products[0].price - Math.floor((p.products[0].price * p.products[0].discount) / 100 )} บาท</h2>
           <p className='line-through'>{p.products[0].price} บาท</p>
       ...
       <div className='flex gap-2 flex-col'>
           ...
           <button onClick={() => dispatch(delete_card_product(p._id)) }  className='px-5 py-[3px] bg-red-500 text-white'>ลบ</button>
       </div>
   </div>
    ...
    <div className='pl-3 md-lg:pl-0 md-lg:mt-5'>
        {
            card_products.length > 0 && <div className='bg-white p-3 text-slate-600 flex flex-col gap-3'>
                ...
                <div className='flex justify-between items-center'>
                    <span>{buy_product_item} รายการ </span>
                    <span>{price} บาท</span>
                </div>
                <div className='flex justify-between items-center'>
                    <span>ค่าจัดส่ง </span>
                ...
                <input className='w-full px-3 py-2 border border-slate-200 outline-0 focus:border-blue-500 rounded-sm' type="text" placeholder='คูปองส่วนลด' />
                <button className='px-5 py-[1px] bg-[#272362] text-white rounded-sm uppercase text-sm'>ใช้</button>
                </div>
                */}
                ...
                <button onClick={redirect} className='px-5 py-[6px] rounded-sm hover:shadow-blue-900/50 hover:shadow-lg bg-blue-950 text-sm text-white uppercase '>
                    ขั้นตอนการชำระเงิน ({buy_product_item})
                </button>
            ...
            : <div>
                <Link className='px-4 py-1 bg-blue-900 text-white rounded-full' to='/shops' >ซื้อเลย</Link>
            </div>
        }
    </div>
    </section>
