...
const SellerToCustomer = () => {
                ...
                <div className='flex w-full h-full relative'>
                    <div className={`w-[280px] h-full absolute z-10 ${show ? '-left-[16px]' : '-left-[336px]'} md:left-0 md:relative transition-all`}>
                        <div className='w-full h-[calc(100vh-177px)] bg-[#ffffff] md:bg-transparent overflow-y-auto'>
                            ...
                            {
            customers.map((c,i) => <Link key={i} to={`/seller/dashboard/chat-customer/${c.fdId}`} className={`h-[60px] mt-2 flex justify-start gap-2 items-center text-black px-2 py-6 rounded-md cursor-pointer bg-[#c8e5f9] `}>
            <div className='relative'>
             <img className='w-[38px] h-[38px] border-green-500 border-2 max-w-[38px] p-[2px] rounded-full' src="http://localhost:3001/images/man.png" alt="" />
    ...
    return ( 
        <div key={i} ref={scrollRef} className='w-full flex justify-end items-center'>
                                    <div className='flex justify-start items-start gap-2 md:px-3 py-2 max-w-full lg:max-w-[85%]'>
                                        <div className='flex justify-center items-start flex-col w-full bg-sky-500 shadow-lg shadow-sky-500/50 text-white py-1 px-2 rounded-sm'>
                                        <span>{m.message}</span>
                                        </div>
                                        <div>
...
<span>Select Customer </span>
                        ...
                        <form onSubmit={send} className='flex gap-3'>
                            <input value={text} onChange={(e) => setText(e.target.value)} className='w-full flex justify-between px-2 border border-slate-700 items-center py-[5px] focus:border-sky-500 rounded-md outline-none bg-transparent text-[#111827]' type='text' placeholder='Input Your Message'/>
                            <button className='shadow-md bg-blue-900 hover:shadow-blue-500/50 font-semibold w-[75px] h-[35px] rounded-md text-white flex justify-center items-center'>Send</button>
                        </form>
