...
const ChatSeller = () => {
    ...
    return (
        <div className='px-2 lg:px-7 py-5'>
            <div className='w-full bg-[#ffffff] px-4 py-4 rounded-md h-[calc(100vh-140px)] shadow-xl'>
                <div className='flex w-full h-full relative'>
                    <div className={`w-[280px] h-full absolute z-10 ${show ? '-left-[16px]' : '-left-[336px]'} md:left-0 md:relative transition-all`}>
                        <div className='w-full h-[calc(100vh-177px)] bg-[#ffffff] md:bg-transparent overflow-y-auto'>
                            <div className='flex text-xl justify-between items-center p-4 md:p-0 md:px-3 md:pb-3 text-[#111827]'>
                                <h2>Sellers</h2>
                            ...
                            {
            sellers.map((s,i) => <Link key={i} to={`/admin/dashboard/chat-sellers/${s._id}`} className={`h-[60px] flex justify-start gap-2 items-center text-black px-2 py-2 rounded-md cursor-pointer ${sellerId === s._id ? 'bg-[#7dd3fc]' : ''}  `}>
            <div className='relative'>
             <img className='w-[38px] h-[38px] border-blue-900 border-2 max-w-[38px] p-[2px] rounded-full' src={s.image} alt="" />
            ...
            <div className='flex justify-center items-start flex-col w-full'>
                <div className='flex justify-between items-center w-full'>
                    ...
                    <div className='w-full md:w-[calc(100%-200px)] md:pl-4'>
                        <div className='flex justify-between items-center'>
                            {
                                sellerId && <div className='flex justify-start items-center gap-3'>
                                    <div className='relative'>
                                    <img className='w-[45px] h-[45px] border-blue-900 border-2 max-w-[45px] p-[2px] rounded-full' src={currentSeller?.image}  alt=""/>
                                    <div className='w-[10px] h-[10px] bg-green-500 rounded-full absolute right-0 bottom-0'></div>
                                </div>
                                <span className='text-black font-semibold'>{currentSeller?.name}</span>
                                </div>
                            }

                            <div onClick={() => setShow(!show)} className='w-[35px] flex md:hidden h-[35px] rounded-sm bg-sky-500 shadow-lg hover:shadow-sky-500/50 justify-center cursor-pointer items-center text-white'>
                            <span><FaList/></span>
                            </div>
                        </div>
                        ...
                        <div className='py-4'>
                            <div className='bg-[#082f49] h-[calc(100vh-290px)] rounded-md p-3 overflow-y-auto'>
        ...
        <div ref={scrollRef} className='w-full flex justify-start items-center'>
                        ...
                        return(
                            <div ref={scrollRef} className='w-full flex justify-end items-center'>
                                    ...
                                    <div className='flex justify-start items-start gap-2 md:px-3 py-2 max-w-full lg:max-w-[85%]'>
                                        <div className='flex justify-center items-start flex-col w-full bg-teal-500 shadow-lg shadow-teal-500/50 text-white py-1 px-2 rounded-sm'>
                                        <span>{m.message}</span>
                                        </div>
                                        <div>
...
<span>Select Seller </span>
                        ...
                        <form onSubmit={send} className='flex gap-3'>
                            <input readOnly={sellerId ? false : true} value={text} onChange={(e) => setText(e.target.value)}  className='w-full flex justify-between px-2 border border-slate-700 items-center py-[5px] focus:border-sky-500 rounded-md outline-none bg-transparent text-[#111827]' type='text' placeholder='Input Your Message'/>
                            <button disabled={sellerId ? false : true} className='shadow-md bg-blue-900 hover:shadow-blue-500/50 font-semibold w-[75px] h-[35px] rounded-md text-white flex justify-center items-center'>Send</button>
                        </form>
